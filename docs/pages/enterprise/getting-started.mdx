...
<Details scopeOnly={true} scope={["oss"]} title="Teleport Enterprise">
This guide is for the self-hosted Teleport Enterprise edition.  Please see the guides within the 
[getting started](../getting-started/) section for deploying a self-hosted Teleport OpenSource edition.
</Details>

<Details scopeOnly={true} scope={["cloud"]} title="Teleport Cloud">
This guide deploys the Teleport Enterprise Auth Service and Proxy Service, which are fully managed in Teleport Cloud. If you are a Teleport Cloud customer, we recommend following our guides for deploying specific services on your Teleport Nodes. Here are some examples:
...
to download the software and [license](../setup/reference/license.mdx) file. You will also need three computers: two servers and
