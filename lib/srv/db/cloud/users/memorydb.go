...
func (f *memoryDBFetcher) getUsersForRegion(ctx context.Context, region string, client memorydbiface.MemoryDBAPI) ([]*memorydb.User, error) {
	getFunc := func() (interface{}, error) {
		...
		for pageNum := 0; pageNum < maxPages; pageNum++ {
			...
			if err != nil {
				return []*memorydb.User{}, libaws.ConvertRequestFailureError(err)
			}
...
func (f *memoryDBFetcher) getUserTags(ctx context.Context, user *memorydb.User, client memorydbiface.MemoryDBAPI) ([]*memorydb.Tag, error) {
	getFunc := func() (interface{}, error) {
		...
		if err != nil {
			return []*memorydb.Tag{}, libaws.ConvertRequestFailureError(err)
		}
...
func (f *memoryDBFetcher) createUser(mdbUser *memorydb.User, client memorydbiface.MemoryDBAPI, secrets libsecrets.Secrets) (User, error) {
	...
	user := &baseUser{
		...
		inDatabaseName:              aws.StringValue(mdbUser.Name),
		modifyUserFunc:              modifyMemoryDBUserFunc(mdbUser, client),
		clock:                       f.cfg.Clock,
...
// modifyMemoryDBUserFunc is a callback to update passwords for MemoryDB user.
func modifyMemoryDBUserFunc(user *memorydb.User, client memorydbiface.MemoryDBAPI) func(context.Context, string, string) error {
	return func(ctx context.Context, oldPassword, newPassword string) error {
		...
		if _, err := client.UpdateUserWithContext(ctx, input); err != nil {
