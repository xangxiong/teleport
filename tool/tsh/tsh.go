...
type CLIConf struct {
	...
	DaemonAddr string
	// DaemonCertsDir is the directory where certs for the daemon communication are stored
...
func Run(ctx context.Context, args []string, opts ...cliOption) error {
	...
	daemonStart.Flag("certsDir", "Directory where certificates for sercure communication are stored").StringVar(&cf.DaemonCertsDir)
